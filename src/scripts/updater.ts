...
readFile(
  `${dataSetting["baseDir"]}/version.json`,
  ...
  function (err, stats) {
    if (err) {
      (async () => {
        var options = {
          uri:
            "https://raw.githubusercontent.com/RingoMar/androyd/master/version.json",
        };
    ...
    let versionNumber = readFileSync(`${dataSetting["baseDir"]}/version.json`, {
      encoding: "utf8",
      flag: "r",
    });
    ...
    (async () => {
      ...
      if (verString < verCloud) {
        ...
        rinConsole(
          "info",
          `Oybot version ${cloudVerson} ready! (Local:${verString})`
        );
      } else {
        updateReady = false;
...
function rinTrain() {
  const python = child_process.spawn(
    `${dataSetting["baseDir"]}\\Artificial\\train.bat`
  );
...
function rinUpdate() {
  const python = child_process.spawn("python", [
    `${process.env.INIT_CWD}\\src\\dic\\rinZip.py`,
    `https://github.com/RingoMar/androyd/releases/download/${updateNumber}/oybot.zip`,
    dataSetting["baseDir"],
  ]);
  python.stdout.on("data", function (data) {
    const stringdata: string = data.toString();
